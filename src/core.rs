...
pub fn lyndon_conjugate<C: Ord + Copy + Clone>(text: &[C]) -> usize {
    ...
    if factors[0] + 1 == n {
        return 0;
    } //@ if the first factor ends at position n, we are done
...
pub fn compute_lcp<T: AsPrimitive<usize> + num::cast::FromPrimitive>(
    plcp: &[u32],
    sa: &[T],
) -> Vec<u32> {
...
impl RandomStringGenerator {
    ...
    pub fn new(r: std::ops::Range<usize>, log_alphabet_size: u8) -> RandomStringGenerator {
        RandomStringGenerator {
            m_range: r,
            m_log_alphabet_size: log_alphabet_size,
        }
    }
}
...
impl Iterator for RandomStringGenerator {
    ...
    fn next(&mut self) -> Option<Vec<u8>> {
        //TODO: not long enough! -> need random strings!
        if self.m_range.start * 2 >= self.m_range.end {
            ...
            for i in 1..(most_significant_bit / self.m_log_alphabet_size) as usize {
                text.push(
                    (((iter_round >> (self.m_log_alphabet_size * i as u8)) & alphabet_mask) + 1)
                        as u8,
                );
            }
            text.push(0);
